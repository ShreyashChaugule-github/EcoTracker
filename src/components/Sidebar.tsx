...
export default function Sidebar({ currentTab, setCurrentTab, userLevel }: SidebarProps) {
  ...
  return (
    <aside id="sidebar-container" className="w-68 bg-white border-r border-slate-200 text-slate-800 flex flex-col justify-between p-4 shrink-0 transition-all duration-300 shadow-xs">
      <div>
        {/* Brand Banner */}
        <div id="brand-header" className="flex items-center gap-3 py-4 mb-4 select-none">
          ...
          <div>
            <div className="flex items-center gap-1.5">
              <span className="font-bold font-sans text-lg tracking-tight text-slate-900">EcoTracker</span>
              <span className="text-[10px] bg-emerald-50 text-emerald-800 font-bold px-1.5 py-0.5 rounded-md border border-emerald-200">AI</span>
            </div>
            <p className="text-[9px] font-mono tracking-wider text-emerald-600 uppercase font-semibold">PROMPTWARS CHAMPION</p>
          </div>
        </div>

        {/* Gamified Level Status */}
        <div id="level-badge-card" className="bg-slate-50 border border-slate-200 p-3 rounded-2xl mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-emerald-700 font-mono mt-0.5">Level {userLevel} Specialist</p>
          </div>
        </div>
        ...
        <nav id="sidebar-navigation" className="space-y-1">
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                <IconComponent className={`w-4 h-4 shrink-0 ${isActive ? "text-emerald-600" : "text-slate-400 group-hover:text-emerald-600"}`} />
      ...
      <div id="powered-taggers" className="bg-slate-50 border border-slate-200 rounded-xl p-3">
        ...
        <div className="flex justify-center items-center gap-2 text-[9px] font-mono text-slate-500 select-none">
          <span className="flex items-center gap-0.5 text-emerald-600"><Cpu className="w-2.5 h-2.5" /> Gemini</span>
          <span className="text-slate-300">•</span>
          <span className="flex items-center gap-0.5 text-blue-600"><Database className="w-2.5 h-2.5" /> Firebase</span>
        </div>
      </div>
    </aside>
  );
