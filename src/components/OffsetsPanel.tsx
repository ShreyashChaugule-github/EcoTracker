...
export default function OffsetsPanel({ userId, onOffsetFunded, showToast }: OffsetsPanelProps) {
  ...
  const projects = [
    { name: "Amazon Reforestation Initiative", costPerTon: 15, rating: "Gold Standard", loc: "Brazil" },
    { name: "High-Efficiency Kenya Cookstoves", costPerTon: 10, rating: "Verra Certified", loc: "Kenya" },
    { name: "Ocean Carbon-Trap Seawalls", costPerTon: 25, rating: "Gold Standard", loc: "Fiji" },
    { name: "Saharan Solar Array Grid Integration", costPerTon: 12, rating: "Verra Certified", loc: "Morocco" }
  ];
  ...
  const executeOffset = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.data) {
        ...
        setFundingAmount("25");
        if (showToast) {
          showToast(`Excellent stewardship! You funded "${selectedProject}" with $${dollars}, mitigating ${kgOffset} kg of CO2 equivalent from the global atmosphere.`, "success");
        } else {
          console.log(`Excellent stewardship! You funded "${selectedProject}" with $${dollars}, mitigating ${kgOffset} kg of CO2 equivalent from global atmosphere.`);
        }
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div id="offsets-panel-container" className="bg-white border border-slate-200 p-6 rounded-3xl h-[calc(100vh-140px)] flex flex-col justify-between overflow-hidden shadow-xs">
        ...
        <div className="flex items-center gap-2.5 border-b border-slate-100 pb-3 mb-4">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-900 flex items-center gap-1.5">
              Carbon Offset Projects <Sparkles className="w-3.5 h-3.5 text-emerald-600 animate-pulse shrink-0" />
            </h3>
            <p className="text-[10px] text-slate-500">Fund global certified ecological restorations to zero out your net footprint</p>
          </div>
        </div>

        {/* Action contributor form */}
        <form onSubmit={executeOffset} className="bg-slate-50 border border-slate-200 rounded-2xl p-4 mb-4 select-none">
          <div className="grid grid-cols-2 gap-3 mb-3 text-xs">
            <div>
              <label className="block text-[10px] font-mono text-emerald-700 uppercase tracking-widest mb-1.5 font-semibold">Ecological Restoration Project</label>
              <select
              ...
              >
                {projects.map((p) => (
                  <option key={p.name} value={p.name}>{p.name} (${p.costPerTon}/ton)</option>
                ))}
              </select>
            </div>
            <div>
              <label className="block text-[10px] font-mono text-emerald-700 uppercase tracking-widest mb-1.5 font-semibold">Contribution Amount ($ USD)</label>
      ...
      <div className="grow overflow-y-auto space-y-4 pr-1 py-1">
        ...
        <div className="space-y-2.5">
          <h4 className="text-[10px] font-mono text-slate-500 uppercase tracking-widest pl-1 select-none font-semibold">Active Certified Programs</h4>
          
          <div className="grid grid-cols-2 gap-3">
            {projects.map((p) => (
              <div key={p.name} className="p-3.5 bg-slate-50 rounded-2xl border border-slate-200 hover:border-emerald-300 transition-all select-none shadow-xs">
                ...
                <div className="flex items-center justify-between text-[10px] font-mono text-emerald-700 mt-2">
                  <span className="bg-emerald-50 border border-emerald-250 rounded px-1.5 py-0.5 font-bold">{p.rating}</span>
        ...
        <div className="space-y-2 pt-2 border-t border-slate-100 select-none">
          <h4 className="text-[10px] font-mono text-emerald-700 uppercase tracking-wider pl-1 font-bold">Contribution History Ledger</h4>
          
          {loading ? (
            <p className="text-[10px] text-slate-400 font-mono text-center">Loading historic records from FireStore...</p>
          ) : error ? (
            <div className="text-red-500 text-[10px] flex items-center gap-1"><AlertCircle className="w-3 h-3" /> {error}</div>
          ) : offsets.length === 0 ? (
            <p className="text-[10.5px] text-slate-500 font-mono italic pl-1">Funding ledger was clean. Ready to purchase first offsets!</p>
          ) : (
            <div className="space-y-1.5 max-h-36 overflow-y-auto">
              {offsets.map((o) => (
                <div key={o.id} className="text-[10.5px] text-slate-605 flex items-center justify-between bg-white px-3 py-2.5 rounded-xl border border-slate-200 shadow-xs">
