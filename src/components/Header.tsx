...
import { Flame, Volume2, VolumeX, Bell, Settings, User, Sparkles, LogOut } from 'lucide-react';
...
export default function Header({ displayName, level, totalXp, streak, onLogout }: HeaderProps) {
  ...
  const xpPercent = Math.min(
    100,
    Math.max(5, Math.floor((currentLevelXpProgress / nextLevelThreshold) * 100))
  );
  ...
  const notifications = [
    {
      id: 1,
      title: 'Commendation!',
      text: 'You completed 3 green action habits in 2 days. +50 XP bonus',
      time: '2 hrs ago',
    },
    {
      id: 2,
      title: 'Insight Alert',
      text: 'Carbon Coach detected 12% lower utility emissions compared to last week.',
      time: '1 day ago',
    },
  ];
  ...
  return (
    <header
      ...
      className="bg-white/95 backdrop-blur-md border-b border-slate-200/80 text-slate-900 py-3.5 px-6 flex items-center justify-between shadow-xs relative z-30"
    >
      ...
      <div className="flex items-center gap-6 grow max-w-xl">
        ...
        <div
          id="streak-indicator"
          className="flex items-center gap-1.5 bg-orange-50/80 border border-orange-200 px-3 py-1.5 rounded-full select-none cursor-pointer hover:bg-orange-100 transition-colors"
        >
        ...
        <div id="xp-slider" className="flex items-center gap-3 grow select-none">
          <span className="text-[10px] font-mono text-emerald-600 font-bold uppercase tracking-wider shrink-0">
            XP
          </span>
      ...
      <div className="flex items-center gap-4 shrink-0">
        ...
        <div className="relative">
          ...
          {showNotifications && (
            <div
              ...
              className="absolute right-0 mt-3 w-80 bg-white border border-slate-200 rounded-2xl shadow-xl p-4 z-40 text-slate-800"
            >
              <div className="flex items-center justify-between border-b border-slate-150 pb-2 mb-2">
                ...
                <span className="text-[9px] font-mono bg-emerald-50 text-emerald-800 px-1.5 py-0.5 rounded-md border border-emerald-200/50">
                  2 New
                </span>
              </div>
              <div className="space-y-3 mt-2">
                {notifications.map((n) => (
                  <div
                    key={n.id}
                    className="text-xs hover:bg-slate-50 p-1.5 rounded-lg transition-colors"
                  >
        ...
        <div id="user-profile-badge" className="flex items-center gap-2.5">
          <div className="text-right select-none">
            ...
            <p className="text-[9px] text-emerald-600 font-mono tracking-wider uppercase mt-0.5 font-bold">
              Active Warrior
            </p>
          </div>
