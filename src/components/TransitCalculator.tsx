...
export default function TransitCalculator({ onAvoidedCarbonLogged, showToast }: TransitCalculatorProps) {
  const [distance, setDistance] = useState<string>("15");
  const [avoidedLogs, setAvoidedLogs] = useState<Array<{ id: string; mode: string; avoided: number; date: string }>>([]);
  ...
  const logCommuteAction = (modeName: string, modeFactor: number) => {
    ...
    if (avoided <= 0) {
      if (showToast) {
        showToast("This mode doesn't save additional carbon compared to our base Standard Gas Car. Choose public transit, walking, or biking to avoid footprint!", "info");
      } else {
        console.warn("This mode doesn't save additional carbon compared to our base Standard Gas Car.");
      }
      return;
    }
    ...
    if (showToast) {
      showToast(`Terrific! Avoiding standard drive by opting for "${commuteModes.find(m => m.name === modeName)?.icon} ${modeName}" saved ${logEntry.avoided} kg of CO2 equivalent, earning you additional XP!`, "success");
    } else {
      console.log(`Terrific! Avoiding standard drive saved ${logEntry.avoided} kg of CO2 equivalent.`);
    }
  };

  return (
    <div id="transit-calculator-panel" className="bg-white border border-slate-200 p-6 rounded-3xl h-[calc(100vh-140px)] flex flex-col justify-between overflow-hidden shadow-xs">
        ...
        <div className="flex items-center gap-2 border-b border-slate-100 pb-3 mb-4">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-900 flex items-center gap-1.5">
              Eco Transit Footprints <Sparkles className="w-3.5 h-3.5 text-emerald-600 animate-pulse shrink-0" />
            </h3>
            <p className="text-[10px] text-slate-500 font-sans">Compare daily travel methods to find avoided footprint credits</p>
          </div>
        </div>
        ...
        <div className="bg-slate-50 border border-slate-200 p-4 rounded-2xl mb-4 text-xs shadow-xs select-none">
          ...
          <div className="flex items-center gap-3">
            <input 
              ...
              required
            />
            <span className="text-slate-500 font-mono text-xs shrink-0 select-none">km one-way</span>
          </div>
        </div>
      </div>
      ...
      <div className="grow overflow-y-auto space-y-3 pr-1 py-1">
        <h4 className="text-[10px] font-mono text-slate-500 uppercase tracking-widest pl-1 mb-2 select-none font-semibold">Comparative Carbon Emissions</h4>
          ...
          return (
            ...
            >
              ...
              {isBetter ? (
                ...
                >
                  <Check className="w-3.5 h-3.5 animate-bounce-slow" /> Avoid {diffFromBaseline.toFixed(1)} kg
                </button>
              ) : (
      ...
      {avoidedLogs.length > 0 && (
        <div className="shrink-0 max-h-24 overflow-y-auto border-t border-slate-100 pt-3.5 mt-3 select-none">
          <h5 className="text-[10px] font-mono text-emerald-700 uppercase tracking-wider mb-2 font-bold">Avoided Travel Credits</h5>
          <div className="space-y-1.5">
            {avoidedLogs.slice(0, 3).map((l) => (
              <div key={l.id} className="text-[10.5px] text-slate-600 flex items-center justify-between bg-white px-3 py-2 rounded-xl border border-slate-200 shadow-xs">
                <span className="flex items-center gap-1 text-slate-700 font-medium"><Leaf className="w-3.5 h-3.5 text-emerald-600" /> Commute by {l.mode}</span>
                <span className="font-mono text-emerald-700 font-bold bg-emerald-50 border border-emerald-200 px-2.5 py-0.5 rounded-md text-[10px]">-{l.avoided} kg Avoided</span>
              </div>
            ))}
          </div>
        </div>
