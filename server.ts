...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const info = handleBackendFirestoreError(
        err,
        OperationType.UPDATE,
        `eco_actions/${actionId}`
      );
...
app.get('/api/offsets/:userId', async (req, res) => {
  ...
  try {
    ...
    const snap = await getDocsWrapper('offsets', q, (items) =>
      items.filter((item) => item.userId === userId).sort((a, b) => b.date.localeCompare(a.date))
    );
    const logs = snap.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
    res.json(logs);
  } catch (err) {
...
app.get('/api/leaderboard', async (req, res) => {
  try {
    const usersQ = query(collection(db, 'users'), orderBy('totalCo2Saved', 'desc'), limit(10));
        ...
        if (
          !combined.some(
            (c) =>
              ...
              (def.displayName.startsWith('Alex') && c.displayName.startsWith('Alex'))
          )
        ) {
          combined.push(def);
        }
